...
_dprintf("nat_rule: start_nat_rules 6.\n");
		 ...
		 */
					...
					)
			{
				...
				unsigned int lanip_new, head_byte, third_byte, dhcp_s_new, dhcp_e_new;
				struct in_addr ip_addr;

				if(!strcmp(current_lan_netmask, "255.255.255.0")){
					//csprintf("# wanduck: CASE_THESAMESUBNET: ready to change the LAN subnet.\n");

					lanip_new = inet_addr(current_lan_ipaddr);
					head_byte = lanip_new&0xFFFF;
					third_byte = (lanip_new>>16)&0xFF;
					if(third_byte >= 254 || third_byte <= 2)
						third_byte = 3;
					else
						third_byte += 1;
					head_byte += (third_byte<<16);

					lanip_new = head_byte+(0x1<<24);
					dhcp_s_new = head_byte+(0x2<<24);
					dhcp_e_new = head_byte+(0xfe<<24);
					ip_addr.s_addr = lanip_new;
					nvram_set("lan_ipaddr", inet_ntoa(ip_addr));
					nvram_set("lan_ipaddr_rt", inet_ntoa(ip_addr));
					ip_addr.s_addr = dhcp_s_new;
					nvram_set("dhcp_start", inet_ntoa(ip_addr));
					ip_addr.s_addr = dhcp_e_new;
					nvram_set("dhcp_end", inet_ntoa(ip_addr));

					sprintf(cmd, "restart_net_and_phy");
					notify_rc_and_wait(cmd);
				}
			}
#endif
