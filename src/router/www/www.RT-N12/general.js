...
function validate_ipaddr_final(o, v){
	...
	if(v == 'dhcp_start' || v == 'dhcp_end' ||
		...
		if((wan_route_x == "IP_Bridged" && wan_nat_x == "0") || sw_mode == "2" || sw_mode == "3"){
			;
			//WAN interface is disabled in AP/Repeater mode, so it wouldn't be compared with the wan ip.
		}
...
function is_portrange(o){
	...
	else if (keyPressed==44){  //"ï¼? can be type in first charAt  ::: 0220 Lock add
		if (o.value.length==0)
			return false;		
		else
			return true;
