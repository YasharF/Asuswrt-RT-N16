...
function showKeyPanel(){
	...
	if($("#sta_encryption").val() == "WEP"){
		$("#wep_auth_mode_field").css("display", "");
		$("#keyindex_input_field").css("display", "");
	}
	else{
		$("#wep_auth_mode_field").css("display", "none");
...
function submitForm(){
	...
	else if (aplist[idx][2] == "Unknown" && aplist[idx][3] == "WEP")
	{
		...
		//alert("sta_auth_mode: " + document.form.sta_auth_mode.value);
		//alert("wep_auth_mode: "+document.form.wep_auth_mode.value);
		document.form.sta_auth_mode.value = document.form.wep_auth_mode.value;
		//alert("sta_auth_mode: " + document.form.sta_auth_mode.value);
...
var sw_mode = '<% nvram_get_x("WLANConfig11b",  "sw_mode"); %>';
	if(sw_mode != 2){
	...
	<div class="LoadingBox_mask" id="LoadingBox_mask">
		...
		<table align="center" class="popup_table" width="400" border="0" id="wl_key_table" style="display:none">
			...
			<tbody>
				...
				<tr id="wep_auth_mode_field">
					<th><#WLANConfig11b_AuthenticationMethod_itemname#></th>
					<td>
      			<select name="wep_auth_mode">
