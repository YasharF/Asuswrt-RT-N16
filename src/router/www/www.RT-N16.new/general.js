...
{alert('<#JS_validrange#> 0 <#BM_alert_to#> ' + document.form.qos_maxbw_x_0.value + '.');
...
{alert('<#JS_validrange#> 0 <#BM_alert_to#> ' + document.form.qos_umaxbw_x_0.value + '.');
...
function validate_range_sp(o, min, max) {
	...
	if(o.value<min || o.value>max) {
		alert('<#JS_validrange#> ' + min + ' <#BM_alert_to#> ' + max + '.');
		o.value = min;
		o.focus();
		o.select();
		return false;
...
document.form.url_time_x_starthour.value = getTimeRange(document.form.url_time_x_1.value, 0);
document.form.url_time_x_startmin.value = getTimeRange(document.form.url_time_x_1.value, 1);
document.form.url_time_x_endhour.value = getTimeRange(document.form.url_time_x_1.value, 2);
document.form.url_time_x_endmin.value = getTimeRange(document.form.url_time_x_1.value, 3);
}
...
function automode_hint(){ //Lock add 2009.11.05 for 54Mbps limitation in auto mode + WEP/TKIP.
	if(document.form.wl_gmode.value == "0" && (document.form.wl_wep_x.value == 1 || document.form.wl_wep_x.value == 2 || document.form.wl_crypto.value.indexOf("tkip") == 0)){
...
function nmode_encryption(){ //Lock add 2009.11.05 for TKIP limitation in n mode.

	var i, cur, algos, wpa_mode, algos_v;
//	var select_index;

//	select_index = document.form.wl_auth_mode.selectedIndex;
...
//	document.form.wl_auth_mode[document.form.wl_auth_mode.selectedIndex].selected = true;
}
...
/*
	else if (s=="LANHostConfig" && v=="jumbo_frame_enable") {
		if (o.value == 0)
...
/****  Lock add in 1127 from OLD UI in index.asp******/
var Add_Flag = "";
var Edit_Flag = "";
var Mkdir_Flag = "";
var Share_Path = "";
