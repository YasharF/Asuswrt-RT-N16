...
function setSelectedFolder(selectedObj){
	var disk_id, part_id, folder_id;
	var sub_item;
	
	disk_id = getDiskIDfromOtherID(selectedObj.id);
	part_id = getPoolIDfromOtherID(selectedObj.id);
	folder_id = getFolderIDfromOtherID(selectedObj.id);
	
	this.selectedDisk = $(disk_id).firstChild.nodeValue;
	this.selectedPool = $(part_id).firstChild.nodeValue;
	
	for(sub_item in clickedFolderBarCode[this.selectedPool])
		if(clickedFolderBarCode[this.selectedPool][sub_item] == folder_id){
			this.selectedFolder = sub_item;
			return;
		}
}
...
function getFolderIDfromOtherID(objID){
	var folder_id_pos, folder_id;
	
	folder_id = objID.substring(1, objID.length);
