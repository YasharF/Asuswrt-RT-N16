...
function checkWord(o,e){
	...
	if((sk>=48 && sk<=57) || (sk>=97 && sk<=105) || sk==0) // 0->other key
	{
		...
		//document.form.temp.value = moveLeft_key;
		//document.form.rtemp.value = moveRight_key;		
	}
...
function IPinputCtrl(obj, t){
	...
	document.getElementById(obj.name+"_div").style.background = (t==0)?"#CCC":"#FFF";
	for(var i=0; i < IP_List.length; i++){
		if(IP_List[i].type == "text"){
			IP_List[i].disabled = (t==0)?true:false;
			IP_List[i].style.background = (t==0)?"#CCC":"#FFF";
		}
	}
}
...
function DomainValidate(o){
	//var domain_reg=/^[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/;
	//var domain_reg=/^[A-Za-z0-9]+\.[A-Za-z0-9]/;
...
function wan_netmask_check_NUI(o) {
	...
	// 2008.03 James. patch for Oleg's patch. }
	{
		...
		fulfillIP(document.form.wan_ipaddr);
		fulfillIP(o);//*/
...
function validate_ipaddr_final(o,v){
	if(v == 'wan_ipaddr'){
		if(o.value.length == 0){    /*Blank.*/
			alert(o.title+"<#JS_fieldblank#>");
			document.getElementById(o.name+"_div").style.border = "2px solid #CE1E1E";
			document.form.wan_ipaddr1.focus();
			return false;
		}
		else if(o.value.indexOf("0") == 0){ /*首字不能為0*/
			document.getElementById(o.name+"_div").style.border = "2px solid #CE1E1E";
			alert(document.form.wan_ipaddr.value + "<#JS_validip#>");
			document.form.wan_ipaddr1.focus();
			return false;
		}		
			...
			document.getElementById(o.name+"_div").style.border = "2px solid #CE1E1E";
			alert(document.form.wan_ipaddr.value + "<#JS_validip#>");
			document.form.wan_ipaddr4.focus();
			return false;
		}
		else if(IP_Validate(o)){    
			if(document.form.wan_ipaddr1.value >= 224){
				alert("<#JS_field_wanip_rule2#>");
				document.form.wan_ipaddr1.focus();
				document.form.wan_ipaddr1.select();
				return false;
			}
			else if(document.form.wan_ipaddr1.value == 127){
				alert(document.form.wan_ipaddr1.value + "<#JS_field_wanip_rule1#>");
				document.form.wan_ipaddr1.focus();
				document.form.wan_ipaddr1.select();
				return false;
			}
	...
	else if(v == 'wan_netmask'){
		var wan_ipaddr1 = document.form.wan_ipaddr1.value;
		if(o.value.length == 0){    /*Blank.*/

			document.getElementById(o.name+"_div").style.border = "2px solid #CE1E1E";
			
			if(confirm(o.title+"<#JS_fieldblank#>\n<#JS_field_fulfillSubmask#>")){
				if((wan_ipaddr1 > 0) && (wan_ipaddr1 < 127)) o.value = "255.0.0.0";
				else if ((wan_ipaddr1 > 127) && (wan_ipaddr1 < 192)) o.value = "255.255.0.0";
				else if ((wan_ipaddr1 > 191) && (wan_ipaddr1 < 224)) o.value = "255.255.255.0";
				else o.value = "0.0.0.0";
				fulfillIP(o);
			}
			document.form.wan_netmask1.focus();
			return false;
		}
		else if(!(IP_Validate(o))){ /*IP格式錯誤*/
			document.getElementById(o.name+"_div").style.border = "2px solid #CE1E1E";
			...
			/*IP格式正確下，其他規則限制。WAN IP必須是LAN IP的子網路。*/
			if(requireWANIP(v) && (
		...
		if(o.value.length > 0){
			...
			else if(o.value == document.form.wan_ipaddr.value){    /* IP格式正確下，其他規則限制。*/
