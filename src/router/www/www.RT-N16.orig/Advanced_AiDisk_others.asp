...
function xfr(){
	...
	if(document.form.samba_workgroup2.value != ""){
		document.form.st_samba_workgroup.value = decodeURIComponent(document.form.samba_workgroup2.value);
		document.form.st_samba_workgroupb.value = encodeURIComponent(document.form.st_samba_workgroup.value);
	}
	else{
		document.form.st_samba_workgroup.value = document.form.samba_workgroup3.value;
		document.form.st_samba_workgroupb.value = encodeURIComponent(document.form.st_samba_workgroup.value);
	}
}
...
function validForm(){
	...
	if(!blanktest(document.form.st_samba_workgroup, "samba_workgroup")){
		document.form.st_samba_workgroup.focus();
		document.form.st_samba_workgroup.select();
		return false;
	}
	
	if(re.test(document.form.st_samba_workgroup.value)){
		alert("<#JS_validchar#>");
	...
	newString = document.form.computer_name.value;
	var re = new RegExp(' ');
	if(re.test(newString[0]) || re.test(newString[newString.length-1])){
		alert("<#JS_storageright#> can't contain space");
		...
		<td bgcolor="#FFFFFF">
			<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3"  class="FormTable">
				...
				<tr>
					...
					<td>
						<input type="text" name="computer_name" id="computer_name" class="input" maxlength="32" size="32" value="">
					</td>
				</tr>
		  	...
		  	<tr>
				...
				<tr>
					<th><a class="hintstyle" href="javascript:openHint(17, 8);"><#ShareNode_FTPLANG_itemname#></a></th>
					<td>
						<select name="ftp_lang" class="input" onChange="return change_common(this, 'Storage', 'ftp_lang');">
							...
							<option value="RU" <% nvram_match_x("Storage", "ftp_lang", "RU", "selected"); %>><#LANG_RU#></option>
 							<option value="CZ" <% nvram_match_x("Storage", "ftp_lang", "CZ", "selected"); %>><#LANG_CZ#></option>
						</select>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						<input type="radio" name="apps_dl_share" value="1" <% nvram_match_x("Storage", "apps_dl_share", "1", "checked"); %>><#checkbox_Yes#>
						<input type="radio" name="apps_dl_share" value="0" <% nvram_match_x("Storage", "apps_dl_share", "0", "checked"); %>><#checkbox_No#>
					</td>
				</tr>
				
				<tr>
					<th><a class="hintstyle" href="javascript:openHint(17, 9);"><#ShareNode_Seeding_itemname#></a></th>
					<td>
						<select name="apps_seed"class="input">
					  	  <option value="1" <% nvram_match_x("Storage","apps_seed", "1","selected"); %>><#WLANConfig11b_WirelessCtrl_button1name#></option>
					  	  <option value="0" <% nvram_match_x("Storage","apps_seed", "0","selected"); %>><#WLANConfig11b_WirelessCtrl_buttonname#></option>
						</select>
					</td>
				</tr>
				<tr>
					<th><a class="hintstyle" href="javascript:openHint(17, 10);"><#ShareNode_MaxUpload_itemname#></a></th>
					<td>
