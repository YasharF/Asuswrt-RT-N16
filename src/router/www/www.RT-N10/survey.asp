...
<title>ASUS Wireless Router <#Web_Title#> - <#AP_survey#></title>
...
//function login_mac_str(){ return '00:15:AF:BA:42:23';}
//var wireless = [['00:15:AF:BA:42:23', '1', '1']];
...
function ajaxGetData(e) { //透過AJAX向aplist.asp取得即時的AP狀態。
...
var times = 15; // times for retry request;
var secs = 100;
...
function ajaxGetUREIP(e) { //Use AJAX to get New IP of RT-N13U from ureip.asp( urelease hook ).
  $.ajax({
    ...
    error: function(xhr) {
      //alert('<#ajax_disconnect#>');
      //setTimeout("ajaxGetUREIP();", 5000);
      setTimeout("ajaxGetUREIP();", 1000);
    },
    success: function(response) {
    	...
    	if(times >= 0){
	    	if(ureIP[0] == ""){  //未取得新IP，則每隔一秒重新request;
					//setTimeout("ajaxGetUREIP();", 5000);
					setTimeout("ajaxGetUREIP();", 1000);
					times--;
				}
    ...
    success: function(response) {
    	...
    	clearTimeout(id_countdown);
			if(navigator.appName.indexOf("Microsoft") >= 0){
				setTimeout("$('#connecting_table tbody td').eq(1).html('<br/><#APSurvey_action_renew_success#>');", 20000);
				setTimeout("location.href = 'http://' + ureIP[0] + '/ure_success.htm';", 25000);
			}
			else{
...
function showList(){
	...
	for(var i=0; i<aplist.length ;i++){
		...
		//use abbr to store BSSID, In/Ad and Orig ssid;
		...
		var ellipsis_ssid = (aplist[i][0].length > 18)?aplist[i][0].substring(0,18)+"...":aplist[i][0];
		...
		if(aplist[i][8] != 0 && aplist[i][0] == ""){ //Hidden SSID but connected.
			...
			html_code += "<span class='ssid'>"+ ellipsis_ssid+ "</span>";  //SSID
...
function bind_event(){ //綁定每個<TR>的click 及 hover event
	...
	$("#aplist_table tbody tr").bind("click", function(e){  // bind click event to <tr> in <tbody>
		...
		document.form.sta_ssid.value = aplist[idx][0];
	...
	$(".ssid").hover(   //Show tooltips of SSID & MAC
		 function(){
		 	var tooltiptext = $(this).parent().attr('abbr');
...
function validateKey(){
		if(document.form.sta_ssid.value == "" && document.form.password.value == ""){
			$("#alert_msg").text("<#APSurvey_Validate_ssid_blank#>")
			document.form.ha_ssid.focus();
			return false;
		}		
		else if(document.form.password.value == ""){
			$("#alert_msg").text("<#APSurvey_Validate_key_blank#>");
			document.form.password.focus();
			return false;
		}
		
		if(aplist[idx][2] == "WPA-PSK" || aplist[idx][2] == "WPA2-PSK"){
...
function submitForm(){
	...
	{
		//document.form.sta_auth_mode.value = "open"; //Can not differ open and shared
	...
	else if (aplist[idx][2] == "WPA2-PSK")
	{
		...
		if (document.form.sta_encryption.value == "TKIP")
			...
			document.form.sta_crypto.value = "aes";
		else if (document.form.sta_encryption.value == "TKIP+AES")
			document.form.sta_crypto.value = "tkip+aes";			
	...
	document.form.ui_triggered.value = "1";
...
<input type="hidden" name="sta_ssid" id="sta_ssid" value="<% nvram_get_x("",  "sta_ssid"); %>">
...
<input type="hidden" name="ui_triggered" value="">
  ...
  <tr>
	...
	<div class="LoadingBox_mask" id="LoadingBox_mask">
		<table align="center" id="LoadingBox" style="display:block;">
			<tr>
				<td><img src="images/load.gif" align="absmiddle" hspace="2" vspace="4"><span id="loading_text"><#APSurvey_action_searching_AP#></span></td>
			</tr>
		</table>
			...
			<tfoot>
				<tr>
					<td height="40" colspan="2">
						...
						<input type="button" value="<#CTL_Add_enrollee#>" onclick="submitForm();" style="margin-left:240px;"/>
