...
int Nbns_query(unsigned char *src_ip, unsigned char *dest_ip, P_IP_TABLE p_ip_tab)
{
    ...
    struct timeval tv1, tv2, timeout={1, 500};
...
SMBretry:
		...
		NMP_DEBUG_M("Operate= %d\n", operate);
				...
				swapbytes16(smb_hdr.Flags2); //2008.10.14 Yau swap for DSL-N13(BigEndian)
                                bzero(&smb_hdr.Pad, 12);
                                smb_hdr.Tid = 0x0000;
                                smb_hdr.Pid = 0xFEFF;
				swapbytes16(smb_hdr.Pid);//2008.10.14 Yau swap for DSL-N13(BigEndian)
                                smb_hdr.Uid = 0x0000;
                                smb_hdr.Mid = 0x0000;
                                ...
                                ByteCount = 0x0062;
				swapbytes16(ByteCount); //2008.10.14 Yau swap for DSL-N13(BigEndian)
				...
				swapbytes16(smb_hdr.Pid); //2008.10.14 Yau swap for DSL-N13(BigEndian)
                                smb_hdr.Uid = 0x0000;
                                smb_hdr.Mid = 0x0010;
				swapbytes16(smb_hdr.Mid); //2008.10.14 Yau swap for DSL-N13(BigEndian)
				...
				swapbytes16(seX_req.MaxMpxCount); //2008.10.14 Yau swap for DSL-N13(BigEndian)
				...
				swapbytes16(seX_req.CaseInsensitivePasswordLength); //2008.10.14 Yau swap for DSL-N13(BigEndian)
                                memcpy(buf+offsetlen, &seX_req.CaseInsensitivePasswordLength, 2);
                                offsetlen += 2; // 53
                                seX_req.CaseSensitivePasswordLength = 0x0001;
				swapbytes16(seX_req.CaseSensitivePasswordLength); //2008.10.14 Yau swap for DSL-N13(BigEndian)
				...
				swapbytes32(seX_req.Capabilities); //2008.10.14 Yau swap for DSL-N13(BigEndian)
