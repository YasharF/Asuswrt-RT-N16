 ...
 *
 * $Id: firewall.c,v 1.2 2008/08/19 03:14:45 james26_jang Exp $
 */
...
start_firewall(void)
{
	...
	if(nvram_match("wl_guest_enable", "1"))
	{
		...
		memset(&filter, 0, sizeof(filter));
		filter.match.state = NETCONF_NEW;
		strcpy(filter.match.in.name, "wl0.1");
		strcpy(filter.match.out.name, nvram_safe_get("lan_ifname"));
		filter.target = log_drop;
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);
		
		strcpy(filter.match.out.name, "wl0.1");
		strcpy(filter.match.in.name, nvram_safe_get("lan_ifname"));
		filter.target = log_drop;
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);
		...
		strcpy(filter.match.in.name, "wl0.1");
		filter.target = log_drop;
		filter.match.ipproto = IPPROTO_TCP;
		filter.match.src.ports[1] = htons(0xffff);
		filter.match.dst.ports[0] = filter.match.dst.ports[1] = htons(80);
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);
		...
		filter.match.ipproto = IPPROTO_TCP;
		filter.match.src.ports[1] = htons(0xffff);
		filter.match.dst.ports[0] = filter.match.dst.ports[1] = htons(atoi(nvram_safe_get("usb_webhttpport_x")));
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);

		filter.match.ipproto = IPPROTO_TCP;
		filter.match.src.ports[1] = htons(0xffff);
		filter.match.dst.ports[0] = filter.match.dst.ports[1] = htons(atoi(nvram_safe_get("usb_ftpport_x")));
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);

		filter.match.ipproto = IPPROTO_TCP;
		filter.match.src.ports[1] = htons(0xffff);
		filter.match.dst.ports[0] = filter.match.dst.ports[1] = htons(atoi(nvram_safe_get("usb_webactivex_x")));
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);
		...
		filter.match.ipproto = IPPROTO_TCP;
		filter.match.src.ports[1] = htons(0xffff);
		filter.match.dst.ports[0] = filter.match.dst.ports[1] = htons(515);
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);

		filter.match.ipproto = IPPROTO_TCP;
		filter.match.src.ports[1] = htons(0xffff);
		filter.match.dst.ports[0] = filter.match.dst.ports[1] = htons(9100);
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);
		...
		filter.match.ipproto = IPPROTO_TCP;
		filter.match.src.ports[1] = htons(0xffff);
		filter.match.dst.ports[0] = filter.match.dst.ports[1] = htons(3838);
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);

#endif

		// accept dhcp packet only.
		memset(&filter, 0, sizeof(filter));
		filter.match.state = NETCONF_NEW;
		strcpy(filter.match.in.name, "wl0.1");
		filter.target = log_accept;
