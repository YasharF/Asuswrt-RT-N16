...
struct HashNode* ILibFindEntry(void *hashtree, void *key, int keylength, int create)
{
	...
	while(done==0)
	{
		...
		{
			#ifdef ASUS_UPNP_DEBUG
			printf("Create an entry: KeyValue=%s\n", (char*)key);
			#endif
			//
...
struct parser_result* ILibParseString(char* buffer, int offset, int length, char* Delimiter, int DelimiterLength)
{
	...
	char* Token = NULL;
	int TokenLength = 0;
	struct parser_result_field *p_resultfield = NULL;
...
void ILibDestructParserResults(struct parser_result *result)
{
	...
	struct parser_result_field *temp = NULL;
...
struct packetheader* ILibParsePacketHeader(char* buffer, int offset, int length)
{
	...
	struct parser_result *_packet = NULL;
	struct parser_result *p = NULL;
	struct parser_result *StartLine = NULL;
	struct parser_result_field *HeaderLine = NULL;
	struct parser_result_field *f = NULL;
	...
	while(HeaderLine!=NULL)
	{
		...
		// Arthur, 050721, DLNA CTT 1.00.07 case 7.8.10.3
		#ifdef ASUS_DLNA_CTT
		if(node!=NULL && (HeaderLine->data[0] == 0x09 || HeaderLine->data[0] == 0x20))
		#else
...
int ILibGetRawPacket(struct packetheader* packet,char **RetVal)
{
	...
	while(node!=NULL)
	{
		...
		// delimiters.
		//
		if(node->FieldDataLength>MAX_HEADER_LENGTH)
		{
			BufferSize += ILibFragmentTextLength(node->FieldData,node->FieldDataLength,"\r\n ",3,MAX_HEADER_LENGTH);
		}
		node = node->NextField;
	}
	//
	...
	*RetVal = (char*)malloc(BufferSize);
	...
	while(node!=NULL)
	{
		...
		memcpy(Buffer+i,": ",2);
		i+=2;
		BufferSize += (node->FieldLength + 2);

		if(ILibFragmentTextLength(node->FieldData,node->FieldDataLength,"\r\n ",3, MAX_HEADER_LENGTH)>node->FieldDataLength)
		{
			// Fragment this
			i2 = ILibFragmentText(node->FieldData,node->FieldDataLength,"\r\n ",3, MAX_HEADER_LENGTH,&temp);
		...
		else
		{
			...
			BufferSize += node->FieldDataLength;
		}
...
/*! \fn ILibGetHeaderLine(struct packetheader *packet, char* FieldName, int FieldNameLength)
	...
	\para
	\b Note: The result must be freed
...
void ILibLifeTime_AddEx(void *LifetimeMonitorObject,void *data, int ms, void* Callback, void* Destroy)
{
	...
	{
		//
		// There are no current triggers, so this is the first,
		// which also means, the Select timeout may not be short enough, so we need
		// to force an unblock, which will then reset the timeout appropriately
		//
	...
	else
	{
		...
		while(temp!=NULL)
		{
			if(ltms->ExpirationTick<=temp->ExpirationTick)
			{
				...
				{
					//
					// This is the shortest trigger, so again, the select timeout
					// may not be short enough, so we need to force unblock, to 
					// recalculate the timeout for the select
					//
					UPnPLifeTime->LM=ltms;
					temp->Prev = ltms;
					NeedUnBlock = 1;
				...
				{
					//
					// This isn't the shortest trigger, so we are gauranteed that the
					// MicroStack thread will unblock in time to reset the timeouts
					//
					ltms->Prev = temp->Prev;
					temp->Prev->Next = ltms;
					temp->Prev = ltms;
...
void ILibLifeTime_Check(void *LifeTimeMonitorObject,fd_set *readset, fd_set *writeset, fd_set *errorset, int* blocktime)
{
	...
	struct LifeTimeMonitorData *Temp, *Temp2, *Temp3, *EVT,*Last=NULL;
	struct ILibLifeTime *UPnPLifeTime = (struct ILibLifeTime*)LifeTimeMonitorObject;
	int nexttick;
	...
	if(UPnPLifeTime->LM!=NULL)
	{
		...
		{
			//
			// Since these are in sorted order, EVT will always point to the first
			// node that needs to be triggered. Last will point to the last node that
			// needs to be triggered. Temp will point to the first node that doesn't
		...
		{
			//
			// We need to create this queue, because it's possible that someone may try to remove
			// this particular node, while we are trying to remove it, which could get tricky. So
			// by creating this queue, we can track it.
			//
			UPnPLifeTime->Reserved = ILibQueue_Create();
		}
		...
		while(EVT!=NULL)
		{
			//ToDo: We may want to check the table below first, before we start triggering
			...
			//
			// After we trigger, we check to see if any that are queued are already going to be
			// fired. Lots of times, a trigger may cause a removal, which may already be happening.
			//
			if(UPnPLifeTime->Reserved!=NULL)
			{
				ILibQueue_Lock(UPnPLifeTime->Reserved);
				Temp3 = NULL;
				//
				// TempObject is a node that was removed while we are triggering events
				// Iterate through all of these
				//
				TempObject = ILibQueue_DeQueue(UPnPLifeTime->Reserved);
				while(TempObject != NULL)
				{
					//
					// We need to iterate through our list of event triggers to see if
					// there is a match. If there is we need to remove it.
					// Save the root at Temp2, so when we're done, we can set EVT back
					// up so we can continue with the event triggers.
					//
					Temp2 = EVT;
					while(EVT!=NULL)
					{
						if(EVT->data==TempObject)
						{
							// Found Match, Remove it instead of triggering it
							if(EVT->Next!=NULL)
							{
							...
							{
								//
								// If we removed the root of the event triggers, we need to 
								// change Temp2, so that when the list is reset, it is set correctly
								//
								Temp2 = EVT->Next;
							}
							EVT->Prev = NULL;
							EVT->Next = NULL;
							//
							// ToDo: Temp3 is initialized to NULL, so it looks like dead
							// code to me... Need to remove it later.
							//
							if(Temp3!=NULL)
							{
								EVT->Next = Temp3;
					...
					EVT = Temp2;
					TempObject = (struct LifeTimeMonitorData*)ILibQueue_DeQueue(UPnPLifeTime->Reserved);
				}
				ILibQueue_UnLock(UPnPLifeTime->Reserved);

				//
				// Since none of the queued items were up to be triggered, we can go ahead and
				// trigger the Destroy event, because they won't be triggered. I noticed
				// that in the ILibLifeTime_Remove, it already triggers the Destroy. However
				// since Temp3 was preinitialized to NULL, I don't think this loop will enter
				// anyways, because I think it's dead code.
				// ToDo: Remove this
				//
				while(Temp3!=NULL)
				{
					Temp2 = Temp3->Next;
					if(Temp3->DestroyPtr!=NULL) {Temp3->DestroyPtr(Temp3->data);}
					free(Temp3);
					Temp3 = Temp2;
				}
			}
		...
		{
			//
			// Since we already triggered a remove, we can throw away the queue, since
			// it's useless now
			//
			ILibQueue_Lock(UPnPLifeTime->Reserved);
			while(ILibQueue_DeQueue(UPnPLifeTime->Reserved)!=NULL);
			TempObject = UPnPLifeTime->Reserved;
			UPnPLifeTime->Reserved = NULL;
			ILibQueue_UnLock(TempObject);
			ILibQueue_Destroy(TempObject);
		}
	}
	else
	{
...
void ILibLifeTime_Remove(void *LifeTimeToken, void *data)
{
	...
	struct LifeTimeMonitorData *first,*last,*evt,*temp;
	...
	while(first!=NULL)
	{
		...
		{
			//
			// Save the next pointer for use at end of loop
			//
			temp = first->Next;
	...
	{
		//
		// If this queue exists, that means a bunch of nodes are being triggered.
		// If we are trying to remove one that is about to be triggered, we need to 
		// stop it from being triggered.
		//
		ILibQueue_Lock(UPnPLifeTime->Reserved);
		ILibQueue_EnQueue(UPnPLifeTime->Reserved,data);
		ILibQueue_UnLock(UPnPLifeTime->Reserved);
	}
...
/*! \fn ILibReadFileFromDisk(char *FileName)
	...
	\returns data read
*/
char *ILibReadFileFromDisk(char *FileName, int *FileLen)
{
	char *RetVal;
	*FileLen = ILibReadFileFromDiskEx(&RetVal,FileName);
	return(RetVal);
}
...
void ILibWriteStringToDisk(char *FileName, char *data)
{
	...
	if(SourceFile!=NULL)
	{
		fwrite(data,sizeof(char),(int)strlen(data),SourceFile);
		fclose(SourceFile);
	}
}
*/
