...
// 2008.06 James. {
		...
		if(nvram_match("wl_wpa_mode", "2") || nvram_match("wl_crypto", "aes")){
			pkt->WSetting.Auth=AUTHENTICATION_WPA_PSK2;
			pkt->WSetting.Encrypt=ENCRYPTION_AES;
		}
		else if(nvram_match("wl_wpa_mode", "0")|| nvram_match("wl_crypto", "tkip+aes")){
...
//		char_to_ascii(tmpbuf,  nvram_safe_get("wl_ssid"));
//		nvram_set_str("wl_ssid2", tmpbuf, sizeof(tmpbuf));
		//nvram_set("wl_ssid2", nvram_safe_get("wl_ssid"));
		...
		if (pkt->WSetting.Auth==AUTHENTICATION_OPEN)
		{
			...
			nvram_set("wl_wpa_psk","");/* Cherry Cho added for removing temporary key used by WSC in 2007/3/8. */
		}
...
int OTSStart(int flag)
{
	...
	{
#if FULL_EZSETUP // Added by Chen-I, 20080201	
		stop_service_main(1);
		start_sdhcpd();
		...
		eval("wl","wsec","off");	// 2008.05 James. use "wl" in WL-500gP V2.
		eval("wl","auth", "0");
		eval("wl","wpa_auth", "0");
		eval("wl","eap","off");
		eval("wl","ssid", "ASUS_OTS");//*/
		/* --- Cherry Cho modified in 2007/2/12. --- */
...
//	if (!DH_compute_key(sharedkey, cpubkey, MAX_DHKEY_LEN, dh)) 
	if (!DH_compute_key(sharedkey, cpubkey, dh)) 
	{
...
//	printf("Data Packet %x %x\n", bs_mode, phdr->OpCode);
	...
	switch(phdr->OpCode)
	{
		case NET_CMD_ID_EZPROBE:
		{
		     ...
		     ezprobe_res->isNotDefault = atoi(nvram_safe_get("x_Setting")) | atoi(nvram_safe_get("wps_config_state")); // for EZSetup to coexist w/ WSC
...
#if FULL_EZSETUP // Added by Chen-I 200802012
