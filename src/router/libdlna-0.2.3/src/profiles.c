...
/*dora ++>>*/
struct video_file_title
{
	char title[125];
	char filename[125];
	char fullpath[125];
};
#define VideoFileMAX	125


struct video_file_title VideoFileTitle1[VideoFileMAX]=
{
		{ NULL, NULL, NULL},
};

struct video_file_title VideoFileTitle2[VideoFileMAX]=
{
		{ NULL, NULL, NULL},
};
struct video_file_title *VideoFileTitle =&VideoFileTitle1[0];
...
#define AudioFileMAX	1000
char  AudioFileNum=0;
struct audio_file_info audioInf[AudioFileMAX];


char VideoControlFlag=0; /*Add Media Information for first parsing */
int  VideoFileTitleVar=0;
int  VideoFileTitleVar1=0;

static void dump_stream_info1(const AVFormatContext *s, const char*fname)	// Jiahao
{
	AVMetadataTag *tag = NULL;

	char *tag_value = NULL;
	char *new_tag_value = NULL;
	...
	while ((tag=av_metadata_get(s->metadata,"",tag,AV_METADATA_IGNORE_SUFFIX)))
	{
		...
		/*Album*/
		if (strcasecmp("TALB", tag->key)==0)		// MP3 ID3v2 header
		{
		#ifdef DEBUG_USHARE
			printf("\n-Add audio information (TALB) into database-\n");
		#endif
			strncpy(audioInf[AudioFileNum].fileAlbum,tag->value,strlen(tag->value));
		}
		else if (strcasecmp("album", tag->key)==0)	// MP3 ID3v1 header
		{
		#ifdef DEBUG_USHARE
			printf("\n-Add audio information (album) into database-\n");
		#endif
//			strncpy(audioInf[AudioFileNum].fileAlbum,tag->value,strlen(tag->value));

			tag_value = iconv_convert_asus (tag->value);
			if (tag_value)
				new_tag_value = tag_value;
			else
				new_tag_value = strdup (tag->value);
			strncpy(audioInf[AudioFileNum].fileAlbum,new_tag_value,strlen(new_tag_value));
		}
		else if (strcasecmp("WM/AlbumTitle", tag->key)==0)
		{
		#ifdef DEBUG_USHARE
			printf("\n-Add audio information (albumtitle) into database-\n");
		#endif
			strncpy(audioInf[AudioFileNum].fileAlbum,tag->value,strlen(tag->value));
		#ifdef DEBUG_USHARE
			printf("\n audioInf[%d].fileAlbum=%s\n",AudioFileNum,audioInf[AudioFileNum].fileAlbum);
		#endif
		}

		/*Author*/
		if (strcasecmp("TPE1", tag->key)==0)		// MP3 ID3v2 header
		{
		#ifdef DEBUG_USHARE
			printf("\n-Add audio information (TPE1) into DataBase-\n");
		#endif
			strncpy(audioInf[AudioFileNum].fileAuthor,tag->value,strlen(tag->value));
		#ifdef DEBUG_USHARE
			printf("\n audioInf[%d].fileAuthor=%s\n",AudioFileNum,audioInf[AudioFileNum].fileAuthor);
		#endif
		}
		else if (strcasecmp("author", tag->key)==0)	// MP3 ID3v1 header
		{
		#ifdef DEBUG_USHARE
			printf("\n-Add audio information (author) into DataBase-\n");
		#endif
//			strncpy(audioInf[AudioFileNum].fileAuthor,tag->value,strlen(tag->value));
			...
			if (tag_value)
				...
				new_tag_value = strdup (tag->value);
			strncpy(audioInf[AudioFileNum].fileAuthor,new_tag_value,strlen(new_tag_value));

		#ifdef DEBUG_USHARE
			printf("\n audioInf[%d].fileAuthor=%s\n",AudioFileNum,audioInf[AudioFileNum].fileAuthor);
		#endif
		}
		else if (strcasecmp("hor", tag->key)==0)
		{
		#ifdef DEBUG_USHARE
			printf("\n-Add audio information (hor) into DataBase-\n");
		#endif
			strncpy(audioInf[AudioFileNum].fileAuthor,tag->value,strlen(tag->value));

		#ifdef DEBUG_USHARE
			printf("\n VideoFileTitle2[%d].fileAuthor=%s\n",AudioFileNum,audioInf[AudioFileNum].fileAuthor);
		#endif
		}

		/*Genre*/
		if (strcasecmp("TCON", tag->key)==0)		// MP3 ID3v2 header
		{
		#ifdef DEBUG_USHARE
			printf("\n-Add audio information (TCON) into DataBase-\n");
		#endif
			strncpy(audioInf[AudioFileNum].fileGenre,tag->value,strlen(tag->value));

		#ifdef DEBUG_USHARE
			printf("\n audioInf[%d].fileGenre=%s\n",AudioFileNum,audioInf[AudioFileNum].fileGenre);
		#endif
		}
		else if (strcasecmp("genre", tag->key)==0)	// MP3 ID3v1 header
		{
		#ifdef DEBUG_USHARE
			printf("\n-Add audio information (genre) into DataBase-\n");
		#endif
//			strncpy(audioInf[AudioFileNum].fileGenre,tag->value,strlen(tag->value));
			...
			if (tag_value)
				...
				new_tag_value = strdup (tag->value);
			strncpy(audioInf[AudioFileNum].fileGenre,new_tag_value,strlen(new_tag_value));
		#ifdef DEBUG_USHARE
			printf("\n audioInf[%d].fileGenre=%s\n",AudioFileNum,audioInf[AudioFileNum].fileGenre);
		#endif
		}
		else if (strcasecmp("WM/Genre", tag->key)==0)
		{
		#ifdef DEBUG_USHARE
			printf("\n-Add audio information (WM/Genre) into DataBase-\n");
		#endif
			strncpy(audioInf[AudioFileNum].fileGenre,tag->value,strlen(tag->value));
...
//#endif /* HAVE_DEBUG */
  ...
  while (p)
  {
    ...
    //printf("\n--match_file_extension Find it (%s)--\n",filename);
...
/*dora ++>>*/
dlna_profile_t *
dlna_guess_media_profile1(dlna_t *dlna, const char *filename, const char*fname)
{
  AVFormatContext *ctx;
  dlna_registered_profile_t *p;
  dlna_profile_t *profile = NULL;
...
//#ifdef DEBUG_USHARE	// Jiahao
//#ifdef HAVE_DEBUG
  dump_format (ctx, 0, NULL, 0);
//#endif /* HAVE_DEBUG */
//#endif		// Jiahao

dump_stream_info1(ctx,fname);/*dora ++*/
