           ...
           av_codecs_t *codecs)
{
 ...
 //dora <<
 //dora >>
    if (codecs->vc->codec_id == CODEC_ID_PNG ||codecs->vc->codec_id == CODEC_ID_BMP ||codecs->vc->codec_id == CODEC_ID_GIF ||codecs->vc->codec_id == CODEC_ID_TIFF )
    {
  		for (i = 0; png_profiles_mapping[i].profile; i++)
    			if (codecs->vc->width  <= png_profiles_mapping[i].max_width &&
        			codecs->vc->height <= png_profiles_mapping[i].max_height)
    ...
    return png_profiles_mapping[3].profile;
