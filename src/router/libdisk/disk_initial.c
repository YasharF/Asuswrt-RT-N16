...
//				memset(ide_model, 0, 64);
				memset(ide_model, 0, sizeof(ide_model));
...
//				memset(target, 0, 32);
				...
				else if(!strcmp(new_disk_info->port, "2")){
...
//	memset(usb_info_file, 0, 32);
	memset(usb_info_file, 0, sizeof(usb_info));
...
extern int check_disk_free(const char *mount_point, u64 *total_kilobytes, u64 *used_kilobytes){
	...
	memset(&fsbuf, 0, sizeof(struct statfs));
	...
	*total_kilobytes = (u64)(total_size/(u64)1024);
	*used_kilobytes = (u64)(used_size/(u64)1024);
...
extern int test_fdisk(const char *device_path, const int new_pool_order, const char *new_pool_size, const char *file_system){
	...
	//int result = system("cat /www/claim_fat32.txt | fdisk /dev/discs/disc0/disc");
