...
get_ftype(char *type)   /* get disk type */
{
        ...
        if(statfs("/tmp/harddisk/part0", &fsbuf))
        {
...
//                   strcpy(productid_g, nvram_safe_get("machine_name"));
					 ...
					 strcpy(temp_productid, nvram_safe_get("productid"));
					 if(!strcmp(temp_productid, "WL500gpv2"))
					 	strcpy(productid_g, "WL-500gP V2");
					 else if(!strcmp(temp_productid, "WL520gu"))
						strcpy(productid_g, "WL-520GU");
					 else
					 	strcpy(productid_g, temp_productid);					
...
//#endif /* #ifdef WL700G */
                        ...
                        get_ftype(ftype);
                        printf("get ftpye is %s\n", ftype);
                        memcpy(ginfo->PrinterInfo, ftype, strlen(ftype));
#endif
		     sendInfo(sockfd, pdubuf_res);
		     return pdubuf_res;		  
