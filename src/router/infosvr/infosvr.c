...
void load_sysparam(void)
{
	...
	strncpy(temp_productid, nvram_safe_get("productid"), sizeof(temp_productid));
	if(!strcmp(temp_productid, "WL500gpv2"))
		strncpy(productid_g, "WL-500gP V2", sizeof(productid_g));
	else if(!strcmp(temp_productid, "WL520gu"))
		strncpy(productid_g, "WL-520GU", sizeof(productid_g));
	else
		strncpy(productid_g, temp_productid, sizeof(productid_g));
...
//    if( (fd=open("/dev/usb/lp0", O_RDWR)) < 0 ) //Someone is opening the usb lp0
	semaphore_wait();
	if(nvram_match("MFP_busy", "2") ||  (fd=open("/dev/usb/lp0", O_RDWR)) < 0 ) //Someone is opening the usb lp0
    {
	semaphore_post();
