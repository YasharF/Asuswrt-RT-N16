...
get_instance_by_wlmac(unsigned char *mac)
{
	...
	for (i = 0; i < WLIFU_MAX_NO_WAN; i++) {
		if (i == 0) {
			...
			sprintf(if_name, "wan%d_ifnames", i);
			ifnames = nvram_get(if_name);
			if (ifnames) {
				if (find_in_list(ifnames, nv_name) ||
				    find_in_list(ifnames, os_name))
...
get_deviceinfo_by_mac(char *mac, wpsdev_info_t *wpdev_info)
{
	...
	if (get_wlname_by_mac(mac, wl_name))
		return -1;

	if (nvifname_to_osifname(wl_name, os_name, sizeof(os_name)) < 0)
		return -1;

	sprintf(prefix, "%s_", wl_name);

	/* get all info from nvram */
	wpdev_info->devname = nvram_get("wps_device_name");
	wpdev_info->mfstring = nvram_get("wps_mfstring");
	wpdev_info->modelname = nvram_get("wps_modelname");
	wpdev_info->modelnum = nvram_get("wps_modelnum");
	wpdev_info->boardnum = nvram_get("boardnum");
	if (nvram_match("wl_wps_reg", "enabled"))
		wpdev_info->builtinreg = 1;
	else
		wpdev_info->builtinreg = 0;

	if (nvram_match(strcat_r(prefix, "wps_config_state", tmp), "1"))
		wpdev_info->scstate = 0x2;
	else
		wpdev_info->scstate = 0x1;

	wl_ioctl(os_name, WLC_GET_BAND, &bandtype, sizeof(bandtype));
	if (bandtype == WLC_BAND_5G)
		wpdev_info->rfband = 0x02;
	else
		wpdev_info->rfband = 0x01;
