 ...
 *
 * $Id: nvparse.c,v 1.34 2008/04/18 02:43:39 Exp $
 */
...
char *proto_conv2(char *proto_name, int idx)
{
        char *proto;
...
char *portrange_ex2_conv2(char *port_name, int idx, int *start, int *end)
{
        char *port, *strptr;
        char itemname_arr[32];
...
char *general_conv2(char *ip_name, int idx)
{
        char itemname_arr[32];
...
//        printf("dbg: port trigger!\n");
        ...
        {
                out_proto = proto_conv2("autofw_outproto_x", i);
                out_port = portrange_ex2_conv2("autofw_outport_x", i, &out_start, &out_end);
                in_proto = proto_conv2("autofw_inproto_x", i);
                in_port = portrange_ex2_conv2("autofw_inport_x", i, &in_start, &in_end);
                desc = general_conv2("autofw_desc_x", i);
